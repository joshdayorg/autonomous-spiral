    ...
    if (dbMessages && dbMessages.length > 0 && messages.length === 0) {
      setMessages(
        dbMessages.map((m) => {
          ...
          const parts: Array<{ type: string; text?: string; toolName?: string; toolCallId?: string }> = [];
                ...
                <div key={message.id} className="space-y-3 animate-fade-in">
                  ...
                  {isDraftMessage && (
                    <div className="text-gray-300 text-sm leading-relaxed">
                      Created {visibleDrafts.length} drafts with different angles. Select one to view the full content.
                    </div>
                  )}
